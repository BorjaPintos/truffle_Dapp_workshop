...
window.App = {
    ...
    getCandidateList: function() {
      ...
      window.contractInstance.getNumOfCandidates().then(function(numOfCandidates) {

        //Por cada número de candidatos iteramos y pedimos la información de cada uno
        for (var i = 0; i < numOfCandidates; i++) {
          //obtenemos la info del candidato candidato
          window.contractInstance.getCandidate(i).then(function(data) {
    ...
    vote: function() {
      ...
      //operación de voto
      window.contractInstance.vote(uid, parseInt(candidateID)).then(function(result) {
    ...
    findNumOfVotes: function() {
      ...
      window.contractInstance.getNumOfCandidates().then(function(numOfCandidates) {
          ...
          for (var i = 0; i < numOfCandidates; i++) {
              ...
              var candidatePromise = window.contractInstance.getCandidate(i)
              //creamos promesa de obtencion de votos de candidato
              var votesPromise = window.contractInstance.totalVotes(i)
    ...
    addCandidate: function(){
      ...
      window.contractInstance.addCandidate(id, party).then(function(result) {
